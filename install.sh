...
CONFIG_FILE="${ORIGINAL_SCRIPT_DIR}/.env"
LOG_DIR="${ORIGINAL_SCRIPT_DIR}/logs"

# --- Source common utilities ---
# shellcheck source=scripts/common/logging.sh
source "${ORIGINAL_SCRIPT_DIR}/scripts/common/logging.sh"

# shellcheck source=scripts/common/os-detection.sh
source "${ORIGINAL_SCRIPT_DIR}/scripts/common/os-detection.sh"

# shellcheck source=scripts/common/load-config.sh
source "${ORIGINAL_SCRIPT_DIR}/scripts/common/load-config.sh"
...
install_relay() {
    ...
    "${ORIGINAL_SCRIPT_DIR}/scripts/relay/setup-relay.sh"
...
install_foreign() {
    ...
    # Install 3x-ui panel
    "${ORIGINAL_SCRIPT_DIR}/scripts/foreign/install-3xui.sh"
    
    # Configure VLESS+Reality inbound
    "${ORIGINAL_SCRIPT_DIR}/scripts/foreign/configure-vless-reality.sh"
    
    # Install WARP if enabled
    if [[ "${ENABLE_WARP:-false}" == "true" ]]; then
        "${ORIGINAL_SCRIPT_DIR}/scripts/foreign/install-warp.sh"
    else
        log_info "WARP installation disabled (ENABLE_WARP=false)"
    fi
    
    # Configure outbounds (direct and optionally WARP)
    "${ORIGINAL_SCRIPT_DIR}/scripts/foreign/configure-outbounds.sh"
    
    # Configure server-side routing
    "${ORIGINAL_SCRIPT_DIR}/scripts/foreign/configure-server-routing.sh"
    
    # Setup reverse proxy if enabled
    if [[ "${ENABLE_REVERSE_PROXY:-false}" == "true" ]]; then
        "${ORIGINAL_SCRIPT_DIR}/scripts/foreign/setup-reverse-proxy.sh"
    else
...
main() {
    ...
    case "${TARGET:-}" in
        relay)
            "${ORIGINAL_SCRIPT_DIR}/scripts/health-check.sh" --relay
            ;;
        foreign)
            "${ORIGINAL_SCRIPT_DIR}/scripts/health-check.sh" --foreign
            ;;
        all)
            "${ORIGINAL_SCRIPT_DIR}/scripts/health-check.sh" --all
            ;;
    esac
