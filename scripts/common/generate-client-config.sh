...
if ! command -v generate_all_client_configs >/dev/null 2>&1; then
    ...
    CLIENT_CONFIG_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    
    # Source logging utilities
    # shellcheck source=logging.sh
    source "${CLIENT_CONFIG_SCRIPT_DIR}/logging.sh"
    
    # Source configuration loading utilities
    # shellcheck source=load-config.sh
    source "${CLIENT_CONFIG_SCRIPT_DIR}/load-config.sh"
    
    # Source configuration validation utilities
    # shellcheck source=validate-config.sh
    source "${CLIENT_CONFIG_SCRIPT_DIR}/validate-config.sh"
    
    # Source client routing generation utilities
    # shellcheck source=generate-client-routing.sh
    source "${CLIENT_CONFIG_SCRIPT_DIR}/generate-client-routing.sh"
...
EOF
    ...
    generate_all_client_configs() {
        ...
        local project_root
        project_root="$(cd "${CLIENT_CONFIG_SCRIPT_DIR}/../.." && pwd)"
    ...
    if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
        ...
        CONFIG_FILE="$(cd "${CLIENT_CONFIG_SCRIPT_DIR}/../.." && pwd)/.env"
